    ...
    while cap.isOpened():
        ...
        if results:
            ...
            if hasattr(result, 'keypoints') and result.keypoints.data.numel() > 0:
                ...
                frame_with_keypoints = result.plot(kpt_line=True)  # Automatically plot keypoints with lines

                # Store both the frame with keypoints and the raw frame
                with frame_lock:
                    current_frame[0] = frame_with_keypoints  # Frame with keypoints
                    current_frame[1] = frame  # Raw frame without keypoints
